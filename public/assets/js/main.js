document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderApkCards = (apksToRender, append = false) => {
        ...
        // Este botón solo se mostrará si hay más APKs de las que se renderizaron inicialmente.
        if (loadMoreButton) {
            // Calcula cuántas APKs se han mostrado después de esta renderización
    ...
    // --- Función para filtrar y ordenar las APKs (la misma que en generate_pages.py) ---
    const getSortedAndFilteredApks = (query = '') => {
        ...
        if (query) {
            ...
            filteredApks = apkList.filter(apk =>
                ...
                (apk.description && apk.description.toLowerCase().includes(lowerCaseQuery)) // Añadir verificación para description
            );
            sectionTitle.textContent = `Resultados de búsqueda para "${query}"`;
        } else {
            sectionTitle.textContent = 'Últimas Actualizaciones';
        }
    ...
    // Asegurarse de que apkList exista antes de intentar usarla
    if (latestUpdatesContainer && typeof apkList !== 'undefined' && apkList.length > 0) {
        const initialApks = getSortedAndFilteredApks().slice(0, apksPerPage);
        renderApkCards(initialApks);
        currentApkIndex = apksPerPage; // Establece el índice para la próxima carga
    ...
    if (loadMoreButton) {
        loadMoreButton.addEventListener('click', () => {
            ...
            renderApkCards(nextApks, true); // Pasar 'true' para añadir al contenedor
    ...
    if (searchForm && searchInput) {
        searchForm.addEventListener('submit', (e) => {
            ...
            // Ajustar el botón "Cargar Más" para los resultados de búsqueda
            if (loadMoreButton) {
    ...
    // Esta función se exporta globalmente para que category.js pueda acceder a ella
