document.addEventListener('DOMContentLoaded', () => {
    ...
    const apksPerPage = 10; // Cantidad de APKs a mostrar al inicio (para el Top 10)
    ...
    const getDisplayApkName = (originalName) => {
        ...
        const maxLength = 25; // "CapCut - Video Editor" tiene 21 caracteres.
    ...
    const renderApkCards = (apksToRender, append = false) => {
        ...
        if (loadMoreButton) {
            ...
            const totalApksInList = getSortedAndFilteredApks(searchInput.value.trim()).length;
            if (totalRendered < totalApksInList) {
    ...
    if (latestUpdatesContainer && typeof apkList !== 'undefined' && apkList.length > 0) {
        ...
        const initialApks = allApks.slice(0, apksPerPage); // Ahora toma 10
        renderApkCards(initialApks);
        currentApkIndex = apksPerPage; 
        
        if (loadMoreButton) {
            const totalApksInList = allApks.length;
            if (currentApkIndex < totalApksInList) {
                loadMoreButton.style.display = 'block';
            } else {
                loadMoreButton.style.display = 'none';
            }
