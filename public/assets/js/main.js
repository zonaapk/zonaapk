document.addEventListener('DOMContentLoaded', () => {
    ...
    // NOTA: La lógica para "Top Aplicaciones Descargadas" (24h, 7d, 30d) requerirá
    // una implementación más avanzada aquí (filtrar y ordenar por `installs` y `updated`
    // en relación con la fecha actual). Por ahora, mantiene la ordenación por fecha de
    // actualización descendente, pero el título se ha cambiado.
    const getSortedAndFilteredApks = (query = '') => {
        ...
        if (query) {
            ...
            // El título de la sección se cambia aquí para reflejar el objetivo SEO
            sectionTitle.textContent = 'Top Aplicaciones Más Descargadas en las Últimas 24 Horas'; 
        }

        // Ordenar por fecha de actualización (la más reciente primero)
        // Esto es un placeholder para la futura lógica de "Top Descargas" real.
    ...
    if (latestUpdatesContainer && typeof apkList !== 'undefined' && apkList.length > 0) {
        ...
        // Ajustar el botón "Cargar Más" después de la carga inicial
        if (loadMoreButton) {
