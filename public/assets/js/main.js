document.addEventListener('DOMContentLoaded', () => {
    ...
    const getDisplayApkName = (originalName) => {
        ...
        // Puedes agregar más reglas aquí para otros nombres si es necesario
        // Ejemplo: Truncar a un número máximo de caracteres si sigue siendo muy largo
        const maxLength = 20; // Límite de caracteres deseado
    ...
    const renderApkCards = (apksToRender, append = false) => {
        ...
        apksToRender.forEach(apk => {
            const displayApkName = getDisplayApkName(apk.name); // Usar la función para el nombre a mostrar
            ...
            apkCol.innerHTML = `
                <div class="card apk-card h-100">
                    ...
                    <div class="card-body d-flex flex-column">
                        <h5 class="card-title" title="${apk.name}">${displayApkName}</h5> <p class="card-text text-muted">${apk.developer}</p>
                        <div class="apk-info mt-auto">
                            ${apk.score ? `<span class="badge badge-success">Score: ${apk.score.toFixed(1)} <i class="fas fa-star"></i></span>` : ''}
                        </div>
                        <a href="/apks/${slugify(apk.name)}-${apk.id}.html" class="btn btn-primary mt-3">Ver Más</a>
                    </div>
                </div>
            `;
    ...
    const getSortedAndFilteredApks = (query = '') => {
        ...
        if (query) {
            ...
            // Título principal según lo solicitado
