document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderCategoryCards = (categories) => {
        ...
        if (categories.length > 0) {
            categories.forEach(category => {
                ...
                categoryCol.className = 'col-md-4 mb-4'; // Usar un tamaño de columna adecuado para las categorías
    ...
    if (typeof apkList !== 'undefined' && apkList.length > 0) {
        if (categoryId) {
            ...
            apkList.forEach(apk => {
                if (apk.genreId && apk.genre) {
                    ...
                    const name = apk.genre; // Mantener el nombre original capitalizado si es posible
                    if (!uniqueGenres[id]) {
            ...
            // Convertir el objeto de géneros únicos a un array para renderizar
        ...
        // Si apkList no está definido o está vacío
