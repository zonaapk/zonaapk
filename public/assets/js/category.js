document.addEventListener('DOMContentLoaded', () => {
    ...
    if (categoryId && typeof apkList !== 'undefined') {
        ...
        const filteredApks = apkList.filter(apk => apk.genre && apk.genre.toLowerCase() === categoryId.toLowerCase());
        
        renderApkCards(filteredApks);
    } else {
        categoryTitle.textContent = 'Categoría no encontrada';
        apksContainer.innerHTML = '<p class="col-12 text-center">Por favor, selecciona una categoría válida desde la página de inicio.</p>';
    }
});